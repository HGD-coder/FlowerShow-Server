...
public class InMemoryContentRepository implements ContentRepository {
    ...
    public List<CardItemDto> findFeedPage(String viewerUserId, int offset, int limit) {
        return pageOf(feed, offset, limit);
    }
    ...
    public List<VideoCardDto> findVideoPage(String viewerUserId, int offset, int limit) {
        return pageOf(videos, offset, limit);
    }

    @Override
    public long countPublishedPublicContent(String id) {
        return feed.stream().anyMatch(item -> item.id().equals(id)) ? 1 : 0;
    }
    ...
    private static <T> List<T> pageOf(List<T> items, int offset, int limit) {
